...
local function SpeedingVehicle()
    ...
    function IsVehicleEmergency(vehicle)
        ...
        return vehicleClass == 18  -- 18 是 GTA V 中緊急車輛的類別代碼
    end

    -- 判斷是否為直升機
    function IsHelicopter(vehicle)
        local model = GetEntityModel(vehicle)
        return IsThisModelAHelicopter(model)
    end

    -- 檢查車輛是否為緊急車輛或直升機
    if IsVehicleEmergency(vehicle) or IsVehicleHelicopter(vehicle) then
        return
    end
